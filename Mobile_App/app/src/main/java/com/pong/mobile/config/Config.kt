...
data class Config(
    ...
    val matchmakingGameServerPortRangeStart: Int = Constants.MATCHMAKING_GAME_SERVER_PORT_RANGE_START
) {
    companion object {
        ...
        fun load(context: Context) {
            ...
            current = Config(
                gameServerHost = prefs.getString(KEY_GAME_SERVER_HOST, current.gameServerHost) ?: current.gameServerHost,
                gameServerPort = prefs.getInt(KEY_GAME_SERVER_PORT, current.gameServerPort),
                matchmakingHost = prefs.getString(KEY_MATCHMAKING_HOST, current.matchmakingHost) ?: current.matchmakingHost,
                matchmakingPort = prefs.getInt(KEY_MATCHMAKING_PORT, current.matchmakingPort)
            )
        }
