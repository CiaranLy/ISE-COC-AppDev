...
sealed class NetworkMessage {
    ...
    data class ConnectRequest(override val messageType: String = "CONNECT") : NetworkMessage()
    data class ConnectResponse(
        override val messageType: String = "CONNECT_RESPONSE",
        val playerId: PlayerId,
        val gameState: GameState? = null
    ) : NetworkMessage()

    data class PaddleUpdate(
        override val messageType: String = "PADDLE_UPDATE",
        val playerId: PlayerId,
        val paddle: Paddle
    ) : NetworkMessage()

    data class GameStateUpdate(
        override val messageType: String = "GAME_STATE_UPDATE",
        val gameState: GameState
    ) : NetworkMessage()

    data class StartGameRequest(override val messageType: String = "START_GAME") : NetworkMessage()
    data class StartGameResponse(
        override val messageType: String = "START_GAME_RESPONSE",
        val gameState: GameState
    ) : NetworkMessage()
